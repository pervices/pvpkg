...
mkdir -p host/build
pushd host/build
%cmake %{?have_neon} -DLIB_SUFFIX="/$(DEB_HOST_MULTIARCH)" -DBoost_DEBUG="ON" -DBoost_NO_BOOST_CMAKE="TRUE" -DBoost_NO_SYSTEM_PATHS="TRUE" -DPKG_LIB_DIR="/usr/lib/uhd" -DBoost_ADDITIONAL_VERSIONS="1.69;1.69.0" -DUHD_RELEASE_MODE="release" -DBOOST_LIBRARYDIR:PATH="/usr/lib64/boost169" -DBOOST_INCLUDEDIR="/usr/include/boost169" -DBoost_INCLUDE_DIR="../../../../../usr/include/boost169"  -DENABLE_CRIMSON_TNG="ON" -DENABLE_CYAN_16T="ON" -DENABLE_CYAN_64T="OFF" -DENABLE_CYAN_P1HDR16T="ON" -DENABLE_CYAN_P1HDR32T="ON" -D Boost_NO_BOOST_CMAKE:BOOL="0" -DENABLE_TESTS="OFF"  -DENABLE_N300="OFF"  -DENABLE_E320="OFF" -DENABLE_USRP1="OFF" -DENABLE_B200="OFF" -DENABLE_X300="OFF" -DENABLE_OCTOCLOCK="OFF" -DENABLE_DOXYGEN="OFF" -DENABLE_USB="OFF" -DENABLE_CYAN_8R="ON" -DENABLE_CYAN_4R4T="ON"  \
 ../
make %{?_smp_mflags}
#make -j1
popd
