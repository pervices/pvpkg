...
if [ -z $TEST_LIST ]; then
	...
	do
		...
		python3 -u ${TEST_FILES[$i]}.py -p $PRODUCT -s $serial_number -o $REPORT_DIR -d $DOCKER_SHA -c 0 1
	...
	do
		...
		python3 -u ${TEST_FILES[$idx]}.py -p $PRODUCT -s $serial_number -o $REPORT_DIR -d $DOCKER_SHA -c 0 1
