...
report = pdf_report.ClassicShipTestReport("tx_uhd_tuning", targs.serial, targs.report_dir, targs.docker_sha, targs.addr)
test_fail = 0
summary_tables = []
...
def test(it, data):
    ...
    tx_lo_offset = it["tx_lo"] - it["center_freq"]
    ...
    for ch, channel in enumerate(vsnk):
        ...
        plt.figure()
        plt.title("Channel {} Rx FFT".format(channels[ch]), fontsize=14)
        ...
        data.append([str(tx_dsp_sci) , str(tx_lo), str(center_freq), str(wave_freq), str(channels[ch]), str(tone_present), str(LO_feedthrough_present),  res])
...
if(targs.product == "Vaunt"):
    main(gen.crimson.hi_band.tx_uhd_tune(), "")
elif(targs.product == "Avery"):
    main(gen.calamine.mid_band.tx_uhd_tune(), "")
