...
def main():
    ...
    duration_s = 10
    test_failed = False
    report = pdf_report.ClassicShipTestReport("gpio", targs.serial, targs.report_dir, targs.docker_sha, targs.addr)
    ...
    if(targs.product == 'Tate'):
        ...
        csrc = crimson.get_src_c(channels, 20e6, 15e6, 0.0, targs.addr) # Does not matter if sink or source is used here.
        ...
        csrc = crimson.get_src_c(channels, int(system_rate / 16), 15e6, 0.0, targs.addr) # Does not matter if sink or source is used here.
        pins = 0x0
        ...
        csrc = crimson.get_src_c(channels, 300e6/16, 15e6, 0.0, targs.addr) # Does not matter if sink or source is used here.
        pins = 0x0
        all = 0xFFFFFFFFFFFFFFFF; # 64bit.
