...
def test(it):
    ...
    summary_table.append([it["description"], "{:.2f}".format(it["tx_rate"]/1e6), str(tx_channels), result_string])
...
if(targs.product == "Vaunt"):
    main(gen.crimson.lo_band().tx_rate(4))
elif(targs.product == "Avery"):
    main(gen.calamine.lo_band.tx_rate(4))
