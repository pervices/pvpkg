...
>>>>>>> e2065895 (SHIPTEST: Initial Commit for creating file parse)
...
<<<<<<< HEAD
def fftValues(x, reals, imags):
    #NOTE: Not actually sure if I need the magnitude atm
    # comp = np.vectorize(magnitude)
    # mag_y = comp(reals, imags)#Make complex
    make_comp = np.vectorize(complex)
    comp_y = make_comp(reals, imags)

    fft_y = abs(np.fft.fftshift(np.fft.fft(comp_y)*np.blackman(len(x))))
...
<<<<<<< HEAD
        ...
        plotted_samples = int(round(1/(wave_freq/sample_rate)*num_output_waves))
