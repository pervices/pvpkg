...
def rx_benchmark(duration=10, rx_rate=65e6, rx_channels="0,1,2,3", drop_threshold=0, seq_threshold=0, multi_streamer=True, rx_otw=True, sc='sc16', rx_cpu="sc16", rx_delay=0):
    print('\n\n\n')
    print("*****************************\n")
...
def tx_benchmark(duration=10, tx_rate=65e6, tx_channels="0,1,2,3", drop_threshold=0, seq_threshold=0, multi_streamer=True, tx_otw=True, sc='sc16', tx_cpu="sc16", tx_delay=5):
    print('\n\n\n')
    print("*****************************\n")
...
def main():
    tx_benchmark()
    tx_benchmark(tx_channels="0")
    tx_benchmark(tx_channels="0,1")
    tx_benchmark(tx_cpu='fc32')

    rx_benchmark()
    rx_benchmark(rx_delay=5)
    rx_benchmark(rx_rate=21666667, rx_cpu='fc32', rx_delay=5)
    rx_benchmark(rx_rate=21666667, rx_cpu='fc32', multi_streamer=False)

    rx_tx_benchmark(multi_streamer=False)
    rx_tx_benchmark(tx_delay=0.5, rx_delay=0)
    rx_tx_benchmark(tx_delay=0.5, rx_delay=0, multi_streamer=False, rx_cpu='fc32', tx_cpu="fc32")
