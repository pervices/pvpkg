...
class TestArgs:
    ...
    def __init__(self, parser=None, testDesc=""):
        ...
        if self.addr is None:
            ...
            # Check for if no device was found
            if self.addr is None:
                log.pvpkg_log_error("TEST_ARGS", "Could not detect the unit's management IP address via uhd_find_devices. You can bypass this stage by explicitly stating the address with -a/--addr")
                sys.exit(1)
